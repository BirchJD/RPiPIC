                  LIST     P = P12F683           ; Current version of GPASM doesn't support P12F1822, so dupe gpasm.

                  INCLUDE  "../INCLUDE/P12F1822.INC"

                  ORG      0x2007               ; Current version of GPASM doesn't support P12F1822, so work around.

                  DW       _FOSC_INTOSC & _CLKOUTEN_OFF & _FCMEN_ON & _WDTE_ON & _PWRTE_ON & _MCLRE_OFF & _BOREN_OFF & _IESO_OFF & _CP_OFF & _CPD_OFF
                  DW       _WRT_OFF & _LVP_ON & _BORV_LO & _STVREN_ON & _PLLEN_OFF


;/*********************************************************************************/
;/* Raspberry Pi PIC Programmer - Example LED Flash Program For Device PIC12F1822 */
...
;/* Constants */
;/*************/
GPIO_LED          EQU      (1 << RA2)           ; GPIO pin allocated for driving an LED.
...
;/**********************************/
                  ...
                  MOVLW    0x03                 ; SELECT REGISTER BANK 6
                  MOVWF    BSR
                  GOTO     INIT
...
;/*******************************/
INIT              CLRF     ANSELA               ; Switch off A/D pins, all pins digital.
                  ...
                  MOVLW    (1 << GIE)|(1 << PEIE)|(1 << IOCIE) ; Intrupt on input change.
